...
///   decoding.) Note that it is possible for this array to be empty, for
///   example, when searching reports inverted matches.
///
/// #### Message: **context**
///
...
///   `submatch` correspond to byte offsets into `lines`. (If `lines` is base64
///   encoded, then the byte offsets correspond to the data after base64
///   decoding.) Note that it is possible for this array to be non-empty, for
///   example, when searching reports inverted matches such that the original
///   matcher could match things in the contextual lines.
///
/// #### Object: **submatch**
///
...
/// which the match occurs (`start` is included, but `end` is not). It has
/// these fields:
///
/// * **match** - An
///   [arbitrary data object](#object-arbitrary-data)
...
impl<W: io::Write> JSON<W> {
    ...
    fn write_message(
        ...
        self.wtr.write(&[searcher.line_terminator()])?;
        Ok(())
    }
}
...
impl<'p, 's, M: Matcher, W: io::Write> Sink for JSONSink<'p, 's, M, W> {
    ...
    fn matched(
        ...
        self.json.write_message(searcher, &msg)?;
        Ok(!self.should_quit())
    }
